               ...
               <table class="table table-striped mt-5">
                   <thead>
                       <tr>
                           ...
                           <th class="whitespace-nowrap text-center">Restricted At</th>
                   ...
                   @foreach($customers as $customer)
                       <tr>
                           ...
                           <td class="whitespace-nowrap text-center">{{$customer->phone_number}}</td>
                            <td class="whitespace-nowrap text-center">{{ $customer->deleted_at->toDayDateTimeString() }}</td>
                           <td class="table-report__action w-56">
                               <div class="flex justify-center items-center">
                                   <div class="flex justify-center items-center">
                                        @can('customer_restore')
                                            <button wire:click="selectItem({{$customer->id}},'restore')" class="flex items-center text-success mr-2">
                                                <i class="fa-regular fa-window-restore w-4 h-4 mr-1"></i>Restore
                                            </button>
                                        @endcan
                                        @can('customer_forcedelete')
                                            <button wire:click="selectItem({{$customer->id}},'delete')" class="flex items-center text-danger">
