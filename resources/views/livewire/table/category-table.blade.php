                        ...
                        <div class="absolute right-0 top-0 mr-5 mt-3 dropdown">
                            ...
                            <div class="dropdown-menu w-40">
                                <div class="dropdown-content">
                                    @can('category_edit')
                                        <button wire:click="selectItem({{ $categories->id }},'edit')" class="dropdown-item w-full"> <i class="fa-solid fa-pen mr-1"></i></i> Edit </button>
                                    @endcan
                                    @can('category_edit')
                                        <button wire:click="selectItem({{ $categories->id }},'change_photo')" class="dropdown-item w-full"> <i class="fa-solid fa-pen mr-1"></i></i> Change Photo </button>
                                    @endcan
                                    @can('category_delete')
                    ...
                    <div class="w-full flex justify-center border-t border-slate-200/60 dark:border-darkmode-400 mt-2"> </div>
                        <div class="flex justify-center text-center lg:text-left p-5 ">
                            @if(!empty($categories->photo))
                                <img src="{{ url('storage/category/'.$categories->photo) }}" data-action="zoom" class="w-full h-56" alt="">
                            @else
