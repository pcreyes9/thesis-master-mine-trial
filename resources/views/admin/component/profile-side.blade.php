        ...
        <div class="relative flex items-center p-5">
            <div class="w-12 h-12 image-fit">
                @if(!empty(Auth::guard('web')->user()->photo))
                    <img src="{{ url('storage/employee_profile_picture/'.Auth::guard('web')->user()->photo) }}" data-action="zoom" class="rounded-full"  alt="Missing Image">
                @else
            ...
            <div class="ml-4 mr-auto">
                ...
                <div class="text-slate-500">
                    @foreach(Auth::guard('web')->user()->getRoleNames() as $name)
                        {{ $name }}
                    @endforeach
                </div>
            </div>
